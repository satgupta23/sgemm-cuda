...
// Two changes from v5. Tile loads move 16 bytes per instruction instead of 4, which
// leaves the sector count unchanged and cuts the number of load instructions by four.
// And the a tile is stored transposed, so that the two thread rows a warp spans land in
// banks ThreadM apart instead of in the same bank.
template <int BlockM, int BlockN, int BlockK, int ThreadM, int ThreadN>
__global__ void sgemm_v6_vectorized(int n, const float *a, const float *b, float *c)
{
    ...
    for (int phase = 0; phase < phases; ++phase) {
        ...
        // The four elements came from one row of a and go to four rows of the transposed
        // tile, which is why this is four scalar stores rather than one vector store.
        ...
        reinterpret_cast<float4 *>(&b_tile[b_load_row * BlockN + b_load_col])[0] = b_vector;
    ...
    // Left scalar. Stores are order N squared against N cubed of arithmetic, so
    // vectorizing them gains nothing measurable and would complicate the edge guard.
    #pragma unroll
    for (int i = 0; i < ThreadM; ++i) {
        #pragma unroll
        for (int j = 0; j < ThreadN; ++j) {
